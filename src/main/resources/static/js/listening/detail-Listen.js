...
function loadSubtitles(lines, audioPlayer, subtitlesList) {
    ...
    lines.forEach(line => {
        ...
        // Tạo div để chứa textSpan + resultSpan
        const wrapperDiv = document.createElement("div");
        wrapperDiv.classList.add("d-flex", "flex-column", "mb-2"); // flex theo chiều dọc (trên -> dưới)

        // Tạo span chứa nội dung phụ đề
        const textSpan = document.createElement("span");
        textSpan.textContent = line.content;
        textSpan.dataset.startTime = parseTime(line.startTime);

        // Gắn textSpan vào trong wrapperDiv
        ...
        micButton.addEventListener("click", async (e) => {
            ...
            accessMicrophoneAndRecord(micButton, wrapperDiv);
        });
...
async function accessMicrophoneAndRecord(micButton, wrapperDiv) {
    try {
        if (!micButton.dataset.isRecording) {
            ...
            mediaRecorder.addEventListener("stop", () => {
                ...
                uploadVoice(audioBlob, micButton, wrapperDiv);
...
async function uploadVoice(audioBlob, micButton, wrapperDiv) {
    const baseUrl = window.location.origin;
    const formData = new FormData();
    formData.append("voice", audioBlob, "recording.wav");

    let resultSpan = wrapperDiv.querySelector(".result-span");
    if (!resultSpan) {
        resultSpan = document.createElement("span");
        resultSpan.classList.add("mt-1", "small", "text-muted", "result-span");
        wrapperDiv.appendChild(resultSpan);
    }
    // Thông báo đang xử lý
    resultSpan.innerHTML = `Đang chấm điểm... ⏳`;
    try {
        ...
        if (response.ok) {
            resultSpan.innerHTML = `<strong>You:</strong> ${result.data}`;
        } else {
